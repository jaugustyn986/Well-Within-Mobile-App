...
- History restores an at-a-glance summary after at least three comparable completed cycles. Every derived tile and bullet uses one common eligible set with an exact following cycle start and shows only raw cycle-length, days-after-Peak, first-mucus Cycle Day, and Peak Day ranges with `N`. Completed-cycle count remains separate so an excluded chart is not erased. History does not show averages, `usual`, normative labels, or a future projection.
