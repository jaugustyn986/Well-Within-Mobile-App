...
- **Action 6:** the 2.1.1 release is staged in App Store Connect. Agreements are current; public policy/support pages are live; privacy answers are published; exact store fields, review notes, and six final-size screenshots are saved; and build 21 is complete in the internal `Team (Expo)` TestFlight group. Remaining work is the short accessibility/comprehension check, an internal install smoke test, and a separate final Submit for Review decision.
